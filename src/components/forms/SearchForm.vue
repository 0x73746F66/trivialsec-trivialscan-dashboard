          ...
          >
            <span class="d-flex align-items-start">
              <Toggle
                ...
                @change="hostToggleChange($event, result.hostname)"
              ...
              <div class="d-flex flex-column">
                ...
                <span
                  ...
                  >Last scanned {{ result.last_scanned_ago }}</span
                >
                <span
                  class="font-xs font-color-light"
                  ...
                  :class="[
                    {
                      'ip-addr-res-container-div':
                        result.ip_addr.length + result.resolved_ip.length > 3,
                    },
                  ]"
                ...
                >
                  <span
                    ...
                    v-for="(ip_addr, index) in result.ip_addr"
                    :key="index"
                  >
                    {{ ip_addr }}
                  </span>
                  <span
                    class="font-xxs font-color-light-60 ip-addr-res"
                    v-for="(ip_addr, index) in result.resolved_ip"
                    :key="index"
                  >
                    {{ ip_addr }}
                  </span>
                ...
                <div class="reports max-w-90">
                  ...
                  >
                    <span class="font-xxs font-color-light"
                      >{{ result.reports.length }} Report(s):</span
                    >
                  </div>
                  <a
                    v-for="(report, index) in result.reports"
                    :key="index"
                  ...
                  >
                    {{ index + 1 }}. Report
                    <IconArrowPrimary
                      v-if="index + 1 < result.reports.length"
                      color="f0f0f099"
