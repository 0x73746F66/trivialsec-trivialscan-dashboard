<template>
  <div
    class="d-flex flex-wrap collapsible-pill-container"
  ...
  >
    <button
      ...
      :aria-controls="sect"
      :key="index"
      @click="toggleCollapsible($event, `${label}-${sect}`)"
    >
      {{ sect }}
    </button>
  </div>
  ...
  <div v-else>
    <span class="font-xl font-color-light-80 text-center w-100 d-block"
...
export default {
  ...
  methods: {
    toggleCollapsible(e, collapsible) {
      this.$emit("section-change", e.target.textContent);
      Array.from(
        document.getElementsByClassName(`collapsible-${this.label}-pill`)
      ).forEach(function (el) {
      ...
      Array.from(
        document.getElementsByClassName(`${this.label}-collapse`)
      ).forEach(function (el) {
        el.classList.remove("show");
      });
      document.getElementById(`${collapsible}`).classList.add("show");
    },
  },
};
</script>
