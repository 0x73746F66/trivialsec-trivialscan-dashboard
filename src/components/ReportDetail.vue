...
<script>
export default {
  props: ['generator', 'version', 'account_name', 'client_name', 'project_name', 'targets', 'date', 'execution_duration_seconds', 'score', 'results', 'certificates', 'results_uri', 'config', 'flags', 'evaluations']
}
</script>
...
<div v-if="generator">
  ...
  <div v-if="evaluations" v-for="evaluation in evaluations">
    <ReportSection v-bind="evaluation">
      ...
      <template #heading>{{ evaluation.rule_id }} {{ evaluation.name }}</template>
      ...
      <p v-if="evaluation.metadata.reason">Reason: {{ evaluation.metadata.reason }}</p>
      <p v-if="evaluation.cvss2"><a :href="'https://nvd.nist.gov/vuln-metrics/cvss/v2-calculator?vector=('+evaluation.cvss2+')'">CVSSv2</a></p>
      <p v-if="evaluation.cvss3"><a :href="'https://nvd.nist.gov/vuln-metrics/cvss/v3-calculator?version=3.1&vector='+evaluation.cvss3">CVSSv3</a></p>
        ...
        <ul>
          <li v-for="threat in evaluation.threats">
            ...
            <p v-if="threat.tactic_url">
              tactic:
              <a :href="threat.tactic_url">{{ threat.tactic_id }} {{ threat.tactic }}</a>
            </p>
            <p v-if="threat.description">{{ threat.description }}</p>
            <p v-if="threat.data_source_url">
              data source:
              <a :href="threat.data_source_url">{{ threat.data_source_id }} {{ threat.data_source }}</a>
            </p>
            <p v-if="threat.technique_url">
              technique:
              <a :href="threat.technique_url">{{ threat.technique_id }} {{ threat.technique }}</a>
            </p>
            <p v-if="threat.technique_description">{{ threat.technique_description }}</p>
            <p v-if="threat.sub_technique_url">
              sub technique:
              <a :href="threat.sub_technique_url">{{ threat.sub_technique_id }} {{ threat.sub_technique }}</a>
            </p>
            <p v-if="threat.sub_technique_description">{{ threat.sub_technique_description }}</p>
          </li>
        </ul>
      </div>
    </ReportSection>
