<template>
  <div class="d-flex flex-row align-items-center justify-content-center">
      ...
      <div class="container">
        <div class="row">
          <EvaluationItems :evaluations="issues" :results="results" :resultsFilter="resultsFilter" />
        </div>
      </div>
    </div>
  ...
  computed: {
    results() {
      return {
        ...
        'fail': this.issues.filter(item => item.result_level === 'fail').length,
        'warn': this.issues.filter(item => item.result_level === 'warn').length,
        'info': this.issues.filter(item => item.result_level === 'info').length,
      }
    },
  }
}
