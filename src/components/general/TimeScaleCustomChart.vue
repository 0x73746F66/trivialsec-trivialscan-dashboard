<template>
    <div>
        <div
            ...
            :class="[
                { show: index == 0, hide: index > 0 },
                `compliance-custom-chart-display-${transformedLabel}`
            ]"
            :key="index"
        ...
        >
            ...
            >
                <option
                    ...
                    :key="`${transformedLabel}-${range}`"
                    :value="`${transformedLabel}-${range}`"
            ...
            <CustomChart
                :chartData="chart"
                :label="`${ranges[index]}-${transformedLabel}`"
            />
        </div>
    </div>
</template>
...
export default {
        ...
        handleComplianceTimeScale(e) {
            Array.from(
                document.getElementsByClassName(
                    `compliance-custom-chart-display-${this.transformedLabel}`
                )
            ).forEach(function (el) {
            ...
            document
                .getElementById(`compliance-${e.target.value}`)
    ...
    computed: {
        transformedLabel() {
            return this.label.replace(/[^A-Z0-9]/gi, '_')
        }
    }
}
</script>
